...
class parserMV extends Serializable {
  ...
  /**
   * Parser a line(String) to a Vector(Sparse Vector).
   *
   * @param conv Array[Map] with the information to parser
   * @param line The string to be parsed
   */
  def parserToVector(line: String): Vector = {
    val activeInd = new ArrayBuffer[Int]
    val features = new ArrayBuffer[Double]

    line.trim.split(",").zipWithIndex.foreach { x =>
      val (atr, ind) = x
      if (atr != "" && atr != "?" && atr != "null" && atr != " ") {
        activeInd += ind
        if (conv(ind).contains("min") && conv(ind).contains("max") && (conv(ind).size == 2)) { //If dictionary have like key (only) min and max is real or integer, else, categorical
          features += (atr.toDouble - conv(ind).get("min").get) / (conv(ind).get("max").get - conv(ind).get("min").get)
        } else if (conv(ind).contains("no-bound")) {
          features += atr.toDouble
        } else {
          features += conv(ind).get(atr).get
        }
      }
    }
    
    
    Vectors.sparse(numFeatures, activeInd.toArray, features.toArray)
  }
  ...
  /**
   * Returns the label closest to the value entered
   */
  def getCategoricalLabel(pos: Int, d: Double): Double = {
    val labels = conv(pos).map(_._2).toArray
    var rest = Double.MaxValue
    var n = -1
    for(i <- 0 until labels.size){
      val r = Math.abs(d - labels(i))
      if(r < rest){
        rest = r
        n = i
      }
  ...
  /**
   * Returns the content of "isCategorical" vector in the "pos" position
   */
  def isAtrCategorical(pos: Int): Boolean = {
    isCategorical(pos)
  }
